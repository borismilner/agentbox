...
func Serve(ctx context.Context, runtimeDir, version string, id proto.Identity) error {
	...
	sdk.AddTool(srv, &sdk.Tool{
		...
		Description: "Create a durable step-by-step code review (a walkthrough) and open it on the human's review board. " +
			"You supply a declarative spec: ordered steps, each with prose and a body of blocks - code citations {path, lines:[from,to]} into the repo at repo_root, snippets, figures, tables and callouts - pinned to a commit; " +
...
type wtCreateIn struct {
	Spec   map[string]any `json:"spec" jsonschema:"the version-1 walkthrough spec object: {version:1, title, repo_root (absolute), pinned (commit sha the citations are true against), base?, diff? (the change's unified diff - the only carrier of added/removed knowledge), out_of_scope?, glossary? ([{term, short, body?, also?}] - definitions the board marks in prose and opens on demand, so they stay out of the reading flow), domains? ([{id, title, blurb}] - groups the board opens one at a time; every step then needs a domain and a domain's steps must be consecutive), steps:[{id, kind: ground|code|none|check, title, purpose, tldr ({bottom, points} - REQUIRED on code and check steps, and the board opens in it), prose (segments; p:true starts a paragraph), domain?, body:[{lead?, and exactly one of: path+lines:[from,to] with notes:[{at:[from,to], text}] | snippet | figure ({svg (viewBox required; colours must be currentColor or a --k-* token) | src (repo-relative image or base64 data: URI), caption?, alt?, wide?}) | table ({head, rows, align?, caption?}) | callout ({tone: note|good|warn|danger, title?, prose})}] - figures, tables and callouts may sit on any step, code citations only on code steps; code is the older name for this array], binds?, close?, checks?, cmds? ([{cmd, expect, recorded}])}]}. Read agentbox://standards/walkthrough first; full field reference in agentbox://manual/agent"`
