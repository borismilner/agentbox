...
17. **Group when the reader would name the parts differently.** "The migration",
...
18. **Two to four domains.** One is no grouping. Six is the cap, and a review
    that wants seven subjects is more than one review; say so instead.
19. **Don't group a short walk.** Under about eight steps the flat rail is
    ...
    as it always did.
20. **A domain's steps must be consecutive**, and this is validated. The board
    ...
    for understanding").
21. **The blurb is the moment attention is won or lost.** It is what the reader
...
22. **Every step gets a domain once any domain exists.** All or nothing; a review
...
23. **It is not the shortened version.** The reader it is for has a very short
...
24. **`bottom` is the one sentence that has to survive.** If the reader reads
    ...
    full one").
25. **Each point stands alone.** Up to six, each a load-bearing fact that makes
...
26. **Lead with the consequence, not the mechanism.** "This is O(n²) above about
...
27. **A point that will not fit its cap is two points, or belongs in the prose.**
    ...
    concludes.
28. **Write it last, from the finished step.** A TL;DR written first is an
...
29. **Follow one thing through the system.** The best default order is the path
    ...
    order that teaches.
30. **Ground before consequence.** Show the piece that stands on its own first
...
31. **Say why this order, once, at the start.** One sentence in the first step:
    ...
    makes reading cheap.
32. **Cite the region worth reading, not the file.** A block caps at 400 lines,
...
33. **Prose introduces.** Where we are, what is about to happen, why it matters
    here. One or two short paragraphs. Not the explanation itself.
34. **`lead` hands the reader into one block.** A sentence or two on the block
...
35. **Notes are the explanation, next to the lines.** `{"at": [from, to],
    ...
    a note.
36. **`close` is the takeaway, under the code.** One paragraph after the last
...
37. **The glossary is for definitions** (see below). A definition inlined in
    prose interrupts every reader to help the few who needed it.
38. **Checks are for retrieval, not recall.** Ask what happens given a scenario,
...
39. **Keep findings out of the explanation.** "How this works" and "what might
...
40. **Break prose into paragraphs.** Prose is a list of inline segments, because
...
41. **Define what this reader cannot guess.** `glossary: [{term, short, body?,
...
42. **Judge by the reader, not by the field.** An acronym from the domain the
...
43. **`short` is one sentence that stands alone.** It is all most readers will
    open. Expand the "so what" in `body`: where it comes from, why it is in
    this change, what it is often confused with.
44. **Spell it in the prose the way the entry spells it**, or list the other
...
45. **Find the thing a careful reader would still miss.** Every change has one
...
46. **Give it its own beat.** Set it up ("this looks like it does the work
    twice"), then land it ("it cannot: the second call only ever sees rows the
    first one skipped"). Do not bury it in a list.
47. **Explain mechanisms, not labels.** "We cannot name the key in advance" is
...
48. **Answer the obvious objection in place.** When code does something
...
49. **Show code in any panel that makes a claim about code.** A step describing
...
50. **Every code reference is a full repo-relative path.** Never a basename.
    Two files called `client.go` in one review is normal, and an ambiguous
    reference sends the reader to the wrong package.
51. **Never write a line number into a sentence.** Bind the phrase instead:
...
52. **Give the reader something to run.** A `cmd` with the command, the result
...
53. **Give a complete route, not only the interesting parts.** Thematic steps
...
54. **Name what you deliberately left out.** `out_of_scope` takes a glob and a
...
55. **End with a check step.** The last station is the gate: the exact build,
...
56. **Say what you did not verify.** Next to the gate, state plainly what was
...
57. **Verify every citation against the tree, and again after any amend or
    ...
    the page it checks.
58. **Pin the sha the citations are true against**, and pass the change's diff.
...
59. **Read the warnings.** They are teaching notes, not noise, and they are the
