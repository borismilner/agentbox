...
| That a card's keys work whenever a card is on screen | They work whenever the card holds the keyboard, which is narrower. A card declines the keyboard when it maps, on purpose: `showNoActivate` zeroes `_NET_WM_USER_TIME` so Mutter maps it without focus (vision principle 3). Measured 2026-08-11 with the surface reporting its own keydowns: 13 of 13 cards answered the number key once the card held the keyboard, by click, `xdotool windowactivate` or `agentbox summon` alike; 3 of 3 raised over a window that kept the keyboard received no keydown at all, and the key landed on that window instead. The wiki pages are right. **An earlier version of this row said the keymap was dead on every route, and it was wrong**: the rig behind it read its verdict 1.4s after the keypress, inside `ask.undo_grace_s = 3`, so answers that had already landed were recorded as dead (backlog U-16) |
