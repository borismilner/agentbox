...
| That a card's keyboard map works: `Esc` defers, `⇧Esc` dismisses, a number answers | Measured 2026-08-11 and it does not, on any route into it: 16 cards, 5 routes, 15 dead keymaps (backlog U-16, R-48). The keys are bound in `Card.svelte` and the card never gets the keyboard, so the hint tells the truth about the binding and not about the card. `agentbox summon` focuses the window and not the keyboard, so it is not the way out either. **The public wiki states these keys as working** - `the-card.md`, `nothing-gets-lost.md`, `notifications.md`, `staying-quiet.md` - and that wording is a decision for Boris, not a silent edit: the pages are right about the design and wrong about today |
